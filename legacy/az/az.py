...
def add_notice():
    with open("az/templates/general/notice.txt", "r") as notice:
...
def template_generate(run=False, file="hello", template="Console", script_name="ConsoleApp", namespace_non_console="KnotsSharp", inherit="MonoBehaviour", app_name="Out"):
    ...
    if template == "CodeBlock":
        ...
        shutil.copyfile("az/templates/Unity/Signal.cs", f"output/{namespace_non_console}/Signal.cs")
        ...
        with open(f"{os.path.basename(script_name)}.cs", "w") as f:
            ...
            f.write("{\n")
            script_name_no_extension = script_name.replace(".oskar", "")
            f.write(f"\tpublic class {script_name_no_extension} : {inherit}\n")
            f.write("\t{\n")
            f.write(f"{code}\n")
            f.write("\t}\n")
            f.write("}\n")
...
def transpile(file, tab=True):
    ...
    with open(file, "r") as f:
        ...
        for line in f:
            ...
            # Convert build to Main
            line = line.replace("__build__", "Main(string[] args)")
            ...
            line = line.replace("unity_create", "Instantiate")
            line = line.replace("unity_destroy", "Destroy")
            ...
            # line = line.replace("str", "__textified__")
...
if __name__ == "__main__":
    ...
    # script name
    name = os.path.basename(file_to_build)#[:-5]
